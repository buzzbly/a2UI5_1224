CLASS zui1224_cl_core_srv_draft DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        draft     TYPE zui1224_if_types=>ty_s_draft
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE zui1224_if_core_types=>ty_s_db.

    METHODS read_info
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE zui1224_if_types=>ty_s_draft.
...
CLASS zui1224_cl_core_srv_draft IMPLEMENTATION.
  METHOD cleanup.

    DATA(lv_four_hours_ago) = zui1224_cl_util=>time_substract_seconds( time    = zui1224_cl_util=>time_get_timestampl( )
                                                                     seconds = 60 * 60 * 4 ).

    DELETE FROM zui1224_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE zui1224_if_core_types=>ty_s_db( id                = draft-id
                                                      id_prev           = draft-id_prev
                                                      id_prev_app       = draft-id_prev_app
                                                      id_prev_app_stack = draft-id_prev_app_stack
                                                      uname             = zui1224_cl_util=>context_get_user_tech( )
                                                      timestampl        = zui1224_cl_util=>time_get_timestampl( )
                                                      data              = model_xml ).

    MODIFY zui1224_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
  ...
  METHOD read.
    ...
    IF check_load_app = abap_true.
      ...
      SELECT SINGLE * FROM zui1224_t_01
        WHERE id = @id
